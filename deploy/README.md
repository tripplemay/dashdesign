# Deploy the baseline cloud on Aliyun (always-on ECS + Docker)

An always-on single ECS box running the FastAPI server + Postgres via Docker
Compose. Simplest reliable shape for a small team: no serverless cold starts,
merge jobs run normally, and setup is console + SSH (no extra CLI needed).

Documents default to a local volume; Postgres runs as a container. Upgrade paths
(managed RDS, OSS) are noted at the end. **Estimated cost:** a 2 vCPU / 2 GB
ECS + a modest disk is roughly ¥60–120/month.

---

## 0. What you provide vs. what's automated

You run the account-gated steps (buy ECS, open ports, SSH). Everything else is
in this folder: `Dockerfile`, `docker-compose.yml`, `.env.example`, `Caddyfile`.

---

## 1. Create the ECS instance (Aliyun console)

1. **ECS → 创建实例** (按量付费 is fine to start; switch to 包月 later):
   - Region: pick one close to your team (e.g. 华东1/杭州). Remember it.
   - Spec: 2 vCPU / 2 GiB (e.g. `ecs.e-c1m1.large`) is plenty.
   - Image: **Ubuntu 22.04** or **Alibaba Cloud Linux 3**.
   - System disk: 40 GiB ESSD.
   - Public IP: assign one (or bind an EIP later).
   - Set a login password (or bind an SSH key pair).
2. **Security group** — allow inbound:
   - `22` (SSH) — restrict to your office/home IP if possible.
   - For the quick HTTP test: `8000` — **restrict Source to your IP**.
   - For production TLS: `80` and `443` (open to `0.0.0.0/0` so Let's Encrypt
     can validate). You can then remove `8000`.
