...
```bash
mkdir -p /var/www/dashdesign-updates
```
...
```nginx
location /updates/ {
    alias /var/www/dashdesign-updates/;
    autoindex off;
...
chown -R deploy:deploy /var/www/dashdesign-updates
chmod 755 /var/www/dashdesign-updates
...
> 若嫌麻烦也可直接用现有 root（`VPS_USER=root`、`VPS_UPDATES_DIR=/var/www/dashdesign-updates`），
